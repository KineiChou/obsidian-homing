...
- `providers/client.ts` 的 `createDecisionClient(transport, secrets, settingsGetter)` 返回统一 `DecisionClient`，按 `provider/endpoint` 路由 Jev、OpenRouter、OpenAI-compatible、Anthropic 或显式 Ollama。非 Jev 提供方先用结构化输出（OpenAI 风格为 strict `response_format: json_schema`，Anthropic 为 `output_config.format`），要求每题返回 `choice` 与覆盖全部候选的整数百分比 `probabilities`；OpenRouter 还要求 `provider.require_parameters`，并识别 HTTP 200 内的服务错误。`parseStructuredResponse` 把总和与 100 相差不超过 20、且 `choice` 为最高项的百分比归一为概率；总和偏离、`choice` 不是最高项、只有旧版 `ranking` 或只有 `choice` 时只保留顺序，并在 `ChoiceAnswer.rankOnly` 标记；存在但格式错误的字段仍拒绝。结构化请求被 HTTP 400 拒绝或内容不可用时，客户端抛出 `format` 错误，由共享调度器按正常额度重试一次兼容模式（OpenAI 风格 `json_object`，Anthropic 纯文本，允许代码围栏）；同一提供方、端点、模型连续两次被拒后，本会话直接使用兼容模式。截断和拒答不重试。`rankOnly` 随归档建议保存；0.2.8 之前保存的非 Jev 建议读取时视为 `rankOnly`。界面通过 `filing/alternatives.ts` 的 `closeAlternatives` 判断接近选项，`rankOnly` 建议不算接近。
