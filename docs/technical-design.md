...
状态：描述当前开发分支实现。本文描述当前代码；测试和宿主验证边界见 [验证记录](validation.md)。官方接口核查于 2026-09-22。
...
| 模型接入 | requestUrl → Jev Choice、OpenAI-compatible 或 Anthropic；模型与端点由设置选择 |
| 索引 | 标题／aliases 压缩前缀树、有限元数据；仅内存 |
| 状态 | loadData/saveData 保存 schema 2 设置、待办路径、最小归档建议和移动记录；本机 local storage 保存自动开关和用量 |
...
1. 订阅 create/modify/rename/delete，依路径边界判断 inbox。恢复原队列并将其他既有笔记列为 waiting，不自动发送；批量入口先预览和勾选，再提交选定路径。
2. 内容稳定 10 秒并且未处于活动编辑状态时分析。手动分析可读活动编辑器最新文本；执行移动必须与已保存内容一致。
3. 快照包含会话笔记身份、路径、变化版本和完整文本 SHA-256。发送正文去除 frontmatter，仅单独提取最多 8 个 tags。默认长文采用最多 12,000 UTF-8 字节的结构摘录，携带原文／发送字符数；全文策略保留超限失败。完整内容指纹不因摘录改变。
4. 默认关闭目录画像。启用后从已有索引增量维护有限标题与标签，在强局部信号下预筛；弱信号保留全量候选。可直接存笔记的父目录、叶目录、空目录平等参与。最多 254 个实际目标直接比较，否则每组最多 64 个、每组提名 3 个，最多 64 组，再统一决选。不能跨组比较概率。
...
自动更新链接设置不在稳定公开契约中；宿主适配器仅在运行时确认 `getConfig` 可调用且返回 `alwaysUpdateLinks === true` 时允许依赖自动改写。否则显式路径入链、移动后解析变化的出链或未知缓存会给出具体拒绝原因。移动前记录相关入链／出链，移动后最多等待约 1 秒验证仍解析到同一文件身份，失败进入 review，不自动回滚。Obsidian 1.13.7 合成库已验证一个带入出链移动、两条链接更新及撤销；其他版本、复杂缓存竞争和外部并发修改仍需验证。

归档建议与执行计划分开：目录版本实变时，仍有效的选中目标可以保留并更新版本；无目标建议可重新排队。配置分别维护 filingRevision 和 linkRevision，仅相应语义字段使任务失效。编辑直接清除队列中的过期归档建议，避免后续事件恢复旧结果。

## 编辑与链接流程

1. 通过 registerEditorExtension / editorInfoField 绑定文件与 Editor；不访问未公开的 `.cm`。各编辑会话维护版本、最多 8 个变化范围与有限忽略记录。
2. 按键更新内存版本、脏区间和计时器，映射未受影响的链接建议；不执行链接检索或全文复制。若同一笔记已有归档建议／分析任务，同时将其待办降级为 waiting。停顿 1 秒并结束 composition 后读取最多 1,200 UTF-16 单元窗口，自动路径只匹配脏区间。
...
5. 周边上下文限定在锚点所在的允许文本范围；排除区中的内容不会因附近有匹配而附带发送。模型选择本次候选或放弃，纯内存缓存有界；缓存键绑定文字、局部上下文、源路径、候选及语义配置，不含 documentRevision 或绝对偏移，命中后绑定新锚点。
6. 确认时核对会话、原文、版本、语法、候选、目标解析、重复链接与格式偏好；重新生成的链接与预览不一致就失效。只有此时可额外扫描当前编辑器全文检查尚未保存的链接。
7. 同一会话的批量确认通过一次 Editor.transaction 插入，逐条返回应用结果与失败原因；普通撤销恢复原文。忽略随编辑映射，当前会话保留；手动重新查找可恢复。插入被撤销后抑制同位置自动推荐。

自动路径不读取候选正文、不 grep 全库、不建立持久索引。启动按 250 篇／4 ms 分批索引元数据；单文件更新做词条差量，目录改名与删除处理子树。保存事件会更新待办状态，不能把整个联网和保存流程描述为零写入。

语法排除已通过标准 Markdown 与 CM6 测试；Obsidian 1.13.7 合成库用本地模拟 HTTP 验证了两条链接批量插入及单次原生 Undo。全部自定义语法、输入法和其他宿主版本仍需测试，不能将节点名启发式或这些局部验收当成完整兼容性证明。
...
每日额度发送前预留，返回后记录 token；超时、取消或退出导致无法确认的用量保留为未知。统计仅代表本机本插件，不能推导账户总账单。默认 100 次包括手动分析、连接检查和重试。自动链接另外受 `floor(日上限 × 0.3)` 子额度约束，每次重试也预留；子额度用完不阻止归档和手动链接。预算失败不会全局暂停，归档队列保留 waiting。

批量请求预估只读文件大小和目录元数据，模拟字节分组／打包，给出不含重试的正常请求粗估并据剩余额度建议数量。预估不读取全库正文，也不代替发送前的真实预算预留。

配置文件为 schemaVersion 2，兼容读取 1，包含 settings、filingQueue、moveJournal。最小归档建议保存完整内容指纹、目标路径或 null、前三候选路径和分值、模型／提示版本、设置指纹、创建时间与可选摘录长度；无正文、请求包、密钥值、会话目录 ID 或持久化索引。坏建议单独降级，未知 schema 或损坏核心数据禁写，加载不覆盖原文件。恢复重新读取所需源笔记校验完整指纹并绑定当次身份，不请求模型、不启动自动分析。自动开关和总／自动链接用量使用 vault 范围本机存储。

跨会话 done 归为 archived；intent 根据两端路径及指纹确认已完成或未执行，无法确定才转 review。人工 acknowledge 可结束 review。完成记录限 100 条，未解决记录不裁剪；只有当前会话成功移动可撤销。
