...
4. 任一版本变化使整个任务失效。结果进入待确认列表，人工改选可使用全部有效目标，不局限于模型提名集合。
...
`data.json` 分为 `schemaVersion`、`settings`、`filingQueue`、`moveJournal`。仅保存必要字段：inbox 路径与子目录开关、排除范围、目录用途、功能开关、密钥名称、模型选择、预算、待办路径及状态、源指纹与移动路径。完整正文、请求包、词条索引、编辑建议与候选摘录只在内存中。
...
- 设置页：inbox 选择、范围与用途、归档／链接开关、凭据选择、发送范围、请求预算。分组大小等实验参数先作为工程常量，不让常规设置变成算法调参界面。
- 侧栏 `ItemView`：归档待办与当前笔记的链接建议两个区域，展示原位置、目标完整路径与预览；提供确认、更换目标、稍后和忽略。列表按有限数量分页，不一次渲染所有待办。
- 目标选择：目录使用支持完整路径搜索的选择弹窗；文件目标使用候选列表。人工选择遵循对应功能的范围限制。
- 状态栏：待办数与暂停／连接状态。普通编辑建议不弹阻塞弹窗，不自动劫持 Enter／Tab。
