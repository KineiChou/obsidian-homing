...
自动更新链接设置不在稳定公开契约中；宿主适配器仅在运行时确认 `getConfig` 可调用且返回 `alwaysUpdateLinks === true` 时允许依赖自动改写。否则显式路径入链、移动后解析变化的出链或未知缓存会给出具体拒绝原因。移动前记录相关入链／出链，移动后最多等待约 1 秒验证仍解析到同一文件身份，失败进入 review，不自动回滚。真实宿主链接改写与缓存收敛尚未验收。
