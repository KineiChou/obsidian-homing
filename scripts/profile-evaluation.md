...
The completed live run is recorded in [the raw report](../docs/evaluations/folder-profiles.json) and [evaluation notes](../docs/folder-profile-evaluation.md). The output file starts as `not-run`, becomes `completed` only after all fourteen case/condition evaluations finish, or `incomplete` on failure. Rows contain only case IDs, selected/expected folder IDs, hit flags, actual HTTP request counts and usage metrics. No credentials, note text, request bodies or raw service responses are logged or written.
