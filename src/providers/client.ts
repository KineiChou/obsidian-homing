...
class RankedDecisionClient implements DecisionClient {
  ...
  async evaluate(batch: ChoiceBatch): Promise<ChoiceBatchResult> {
    ...
    let secret: string | null; try { secret = this.secrets.get()?.trim() ?? null; } catch { throw new OrganizerError('authentication', '无法读取 API 密钥，请重新选择凭据。'); }
    const host = new URL(endpoint).hostname;
    if (!secret && !(this.provider === 'openai-compatible' && ['localhost', '127.0.0.1', '[::1]'].includes(host))) throw new OrganizerError('authentication', '请先选择有效的 API 密钥。');
