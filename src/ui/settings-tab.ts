...
export function renderSettings(container: HTMLElement, app: App, controller: OrganizerController): void {
  ...
  let initialSetup = !controller.enabled();
  ...
  enable.addButton(control => control.setButtonText(controller.enabled() ? '检查连接' : '启用归档建议').setCta().onClick(async () => {
    ...
    try { await controller.testConnection(); if (initialSetup) { controller.setEnabled(true); initialSetup = false; } enable.setName('连接正常'); control.setButtonText('检查连接'); status.textContent = '连接正常。'; }
  ...
  new Setting(advanced).setName('每日分析请求上限').setDesc(`本机今日已预留 ${usage.requests} 次，已返回 ${usage.inputTokens} 输入 token，${usage.unknownRequests} 次用量待确认。包含手动分析和重试，请求次数不等于费用。`).addText(input => { input.setValue(String(controller.settings().dailyRequestLimit)); input.inputEl.type = 'number'; input.inputEl.min = '1'; input.inputEl.max = '10000'; input.inputEl.addEventListener('change', () => change({ dailyRequestLimit: Number(input.getValue()) })); });
...
class FolderRuleModal extends Modal {
  ...
  onOpen(): void {
    ...
    const save = button(this.contentEl, '保存', () => {
      ...
      save.disabled = true;
      void this.controller.saveSettings({ folderRules: [...this.controller.settings().folderRules.filter(item => item.path !== this.path), rule] }).then(() => this.close()).catch(error => { status.textContent = messageFor(error); save.disabled = false; });
    }, true);
  }
}
